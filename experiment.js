...
var racket;
var orbitcam;
var ball, ballradius, ballmass, ballvelocity;
var thefloor;
var theboundary;
var font;
var touch, count;
...
function loadExperimentElements()
{
    ...
    PIEsetExperimentTitle("Force can change value of speed");
    ...
    PIEscene.background=new THREE.Color( "#3ef0e3" );
    
    geometry=new THREE.SphereGeometry(ballradius, 32, 32);
    material=new THREE.MeshPhongMaterial({color:0xffffff});
    ball=new THREE.Mesh(geometry, material);
    ball.position.set(0, 0+ballradius, 0);
	ball.xvel=ball.zvel=0;
	PIEaddElement(ball);
    
    loader=new THREE.JSONLoader();
    racket = new THREE.Mesh(new THREE.BoxGeometry(1,10,1), new THREE.MeshBasicMaterial({color:"brown"}));
    racket.scale.y = 0.09;
    racket.scale.z=racket.scale.x = 0.05;
    //racket.translation = geometry.center();
    racket.castShadow=racket.receiveShadow=true;
    racket.position.set(ballradius+0.25, ballradius+0.3, 0);
    PIEaddElement(racket);
    //racket.rotation.set(-Math.PI/2, Math.PI/2, 0);
    racket.mass=2, racket.velocity=5;
    PIEdragElement(racket);
	PIEsetDrag(racket, hamdrag);
    PIErender();
    ...
    tex.load("floor1.jpg", function(texture){
        geometry=new THREE.BoxGeometry(0.05, 0.35, 4.1);
        material=new THREE.MeshPhongMaterial({map:texture});
        for(var i=0; i<2; i++){
            theboundary[i]=new THREE.Mesh(geometry, material);
            PIEaddElement(theboundary[i]);
        }
        theboundary[0].position.set(-2.5, 0.2, 0);
        theboundary[1].position.set(2.5, 0.2, 0);

        geometry=new THREE.BoxGeometry(0.05, 0.35, 5);
        for(var i=2; i<4; i++){
            theboundary[i]=new THREE.Mesh(geometry, material);
            PIEaddElement(theboundary[i]);
            theboundary[i].rotation.y=Math.PI/2;
        }
        theboundary[2].position.set(0, 0.2, -2);
        theboundary[3].position.set(0, 0.2, 2);

        geometry=new THREE.BoxGeometry(5, 0.05, 4.1);
...
function updateExperimentElements(t, dt){
    ...
    if(racket.position.x*racket.position.x + racket.position.z*racket.position.z > (ballradius+ 0.15)*(ballradius + 0.15))
    {
        ballvelocity=(racket.velocity*racket.mass)/ballmass;
        ball.xvel=Math.cos(theta)*ballvelocity;
        ball.zvel=Math.sin(theta)*ballvelocity;
        x=racket.position.x, z=racket.position.z;
        racket.position.x-=0.1*x*racket.velocity/4;
        racket.position.z-=0.1*z*racket.velocity/4;
        ...
        if(touch)
        {
            ...
            text=new THREE.Mesh(new THREE.TextGeometry("Speed of Ball is "+a.toFixed(2)+"m/s",{font:font,size:1/16,height:.01,material:0}),new THREE.MeshLambertMaterial({color:"blue"  }));
            PIEaddElement(text);
            PIErender();
            text.position.set(-0.5,2,2.5);
            
            a = a>=10? a.toPrecision(4) : a.toPrecision(3);
        }
        
        if((x1>=0 && z1<0) || (x1>=0 && z1>=0)){
            ball.position.x=ball.position.x-ball.xvel*dt/1000; 
            ball.position.z=ball.position.z-ball.zvel*dt/1000;
        }
        else{
            ball.position.x=ball.position.x+ball.xvel*dt/1000; 
            ball.position.z=ball.position.z+ball.zvel*dt/1000;
        ...
        if(ball.position.x-ballradius<-2.5){
            ball.position.x=-2.5+ballradius;       
            ball.xvel*=-1;
        }
        if(ball.position.x+ballradius>2.5){
            ball.position.x=2.5-ballradius;
        ...
        if(ball.position.z-ballradius<-2){
            ball.position.z=-2+ballradius;       
            ball.zvel*=-1;
        }
        if(ball.position.z+ballradius>2){
            ball.position.z=2-ballradius;
...
function mass(){
    ballmass=PIEgetInputSlider("Mass of Ball");
    ballradius=(ballmass+3)/40.0;

    PIEscene.remove(ball);
    geometry=new THREE.SphereGeometry(ballradius, 32, 32);
    material=new THREE.MeshPhongMaterial({color:0xffffff});
    ball=new THREE.Mesh(geometry, material);
    ball.position.set(0, 0.0+ballradius, 0);
    PIEaddElement(ball);
...
function initialiseControls(){
    ...
    PIEaddInputSlider("Mass of Ball", 1, mass, 1, 5, 0.1);
    PIEaddInputSlider("Force from racket", 5, function(){ racket.velocity=PIEgetInputSlider("Force from racket"); }, 1, 10, 0.1);
}
...
function resetExperiment(){
    ...
    PIErender();
    racket.visible=true;
    //PIEremoveElement(racket);
    racket.position.set(ballradius+0.25, ballradius+0.3, 0);
    //racket.rotation.set(Math.PI/2, -Math.PI/2, 0);

    PIEremoveElement(text);
    PIErender();
    PIEchangeInputSlider("Force from racket", 5);
    PIEchangeInputSlider("Mass of Ball", 1);
...
{
    helpContent="";
    helpContent = helpContent + "<h2>Force can change value of Speed help</h2>";
    helpContent = helpContent + "<h3>About the experiment</h3>";
    helpContent = helpContent + "<p>Experiment shows motion of ball when ball is hit with Racket.</p>";
    helpContent = helpContent + "<h3>Animation control</h3>";
    helpContent = helpContent + "<p>The top line has animation controls. There are two states of the experiment.</p>";
    helpContent = helpContent + "<h3>The setup stage</h3>";
    helpContent = helpContent + "<p>You can control the following:</p>";
    helpContent = helpContent + "<ul>";
    helpContent = helpContent + "<li>You can choose the Force exerted by Racket on Ball</li>";
    helpContent = helpContent + "<li>You can choos the mass of the ball</li>";
    helpContent = helpContent + "</ul>";
    helpContent = helpContent + "<p>Once you setup the experiment, you can enter the animation stage by clicking the start button</p>";
    helpContent = helpContent + "<h3>The animation stage</h3>";
    helpContent = helpContent + "<p>In the animation stage, the ball will bounce around obeying the laws of physics and elasticity is assumed 1.</p>";
    helpContent = helpContent + "</ul>";
...
{
    infoContent =  "";
    infoContent = infoContent + "<h2>Force can change value of Speed Information</h2>";
    infoContent = infoContent + "<h3>About the experiment</h3>";
    infoContent = infoContent + "<p>Experiment shows change in velocity of ball after being hit by racket from left or right side</p>";
    infoContent = infoContent + "<p>This follows the theorem that change in momentum is equal to Impulse</p>";
    infoContent = infoContent + "<p>mass of the ball*velocity of the ball=Force exerted*impact time of racket on ball</p>";
    infoContent = infoContent + "<h3>Basic Information</h3>";
    infoContent = infoContent + "<p>Time of impact is assumed very small</p>";
    infoContent = infoContent + "<p>Speed of ball will depend on mass of ball and force exerted on ball</p>";
    infoContent = infoContent + "<p>As the surface is smooth so friction force is 0</p>";
    infoContent = infoContent + "<p>Coefficient of elasticity after being hit with walls is 0 </p>";
